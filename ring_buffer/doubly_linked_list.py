...
class DoublyLinkedList:
    ...
    def add_to_head(self, value):
        ...
        # incrementing the list by 1 
        self.length += 1
        # if list is currently empty, add a node in that case
        if not self.head and not self.tail:
            self.head = new_node
            self.tail = new_node
        else:
            # if the list has elements in it
            # make new node's value point to the current head
            new_node.next = self.head
            # make the old head point to the new node, which is now infront (the very first value) 
            self.head.prev = new_node
            # make new node a new head 
            self.head = new_node
